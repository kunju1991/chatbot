	...
	return Controller.extend("ui.ui.controller.XOXO", {
		...
		onInit: function () {
			...
			this.oModel.setData({
				"GetHospitalData": {
					"branches": [{
						"branchID": "1",
						"branchName": "XOXO Charity Hospital",
						"branchLocation": "Whitefield"
					}, {
						"branchID": "2",
		...
		respondTo: function (message) {
			...
			if (msgLength > 0) { //if user has inputted message then
				...
				$.ajax({
					...
					scheme: "https",
					headers: {
						"Authorization": "Token 633b008bbdf606b7fafca9fcccb230af",
						"x-uuid": that.uuid
					},
		...
		getBot: function () {
			...
			$.ajax({
				...
				headers: {
					"Authorization": "0c74149d0a397ca0c2dca311b6a66d5f"
				},
				success: function (data) {
