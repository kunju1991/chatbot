sap.ui.jsview("ui.ui.view.XOXO", {
	...
	getDetailPage: function(oController) {
		if (!this.oDetailPage) {
			...
			//creating tab bar
			oContent.addItem(new sap.m.IconTabFilter("iSolution", {	text: "Branches",height: "100%"}));
			oContent.addItem(new sap.m.IconTabFilter({text: "Doctors",height: "100%"}));
			oContent.addItem(new sap.m.IconTabFilter({text: "Patient",height: "100%"}));
	...
	createSidePanel2: function(){
		...
		oContent2.addItem(new sap.tnt.NavigationListItem({icon: "sap-icon://time-entry-request",tooltip:"Doctor's time enquiry request list"}));
		sideNav.setItem(oContent2);
